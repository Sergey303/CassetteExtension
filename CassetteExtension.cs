...
namespace factograph
{
    public static class CassetteExtension
    {
        ...
        public static Uri MakePhotoPreviews(this Cassette cassette, XElement iisstore, string regimes)
        {
            ...
            {
                factograph.LOG.WriteLine(exc.Message + " Не загрузился файл " + iisstore.ToString());
                return null;
            }
            ...
            if(regimes.Contains('z'))
            {
                ...
                // тестирование
                Archive.Sarc.Create(destinationPath + ".sarc2", new FileSystemInfo[] 
                {
            ...
            if(docIsNew)
            {
                ...
                if(docTag == ONames.TagPhotodoc)
                {
                    ...
                    if(_source != null)
                        ...
                        catch(Exception exep) { factograph.LOG.WriteLine(exep.Message +  " Не удалось зафиксировать Exif информацию по документу " + iisstore.ToString()); }
                }
